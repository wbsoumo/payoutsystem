...
@section('page_title', 'View Support Ticket')
...
<div class="max-w-4xl mx-auto space-y-6">
    ...
    <div class="space-y-4">
        ...
        <div class="bg-white border border-slate-200 rounded-3xl p-6 shadow-sm space-y-3">
            <div class="flex justify-between text-xs border-b border-slate-100 pb-2">
                <span class="font-bold text-slate-900">{{ $ticket->merchant->company_name }} (You)</span>
        ...
        @if($ticket->replies)
            @foreach($ticket->replies as $reply)
                ...
                @endphp
                <div class="rounded-3xl p-6 shadow-sm border space-y-3 {{ $isAdmin ? 'bg-blue-50/50 border-blue-100 mr-12' : 'bg-white border-slate-200 ml-12' }}">
    ...
    @if($ticket->status !== 'closed')
        <div class="bg-white border border-slate-200 rounded-3xl p-6 shadow-sm space-y-4">
            <h3 class="font-bold text-slate-900 text-sm">Post a Reply</h3>
            <form action="{{ route('merchant.tickets.reply', $ticket->id) }}" method="POST" class="space-y-4">
                @csrf
                <div class="space-y-1">
